...
namespace World {
    ...
    void World::GenerateDiorama(
        int widthChunks, 
        int heightChunks, 
        int depthChunks, 
        uint32_t seed, 
        const Render::VoxelCoreRegistry& coreRegistry,
        const Game::BlockConfigRegistry& gameRegistry) noexcept 
    {
        m_coreRegistryReference = &coreRegistry;
        ...
        const float totalWidthWorld = static_cast<float>(m_widthChunks * CHUNK_SIZE) * BLOCK_WIDTH;
        const float totalDepthWorld = static_cast<float>(m_depthChunks * CHUNK_SIZE) * BLOCK_DEPTH;
        ...
        int totalWorldHeight = m_heightChunks * static_cast<int>(CHUNK_SIZE);

        StrataLayerPalette dynamicPalette{
            .airID          = 0,
            .surfaceID      = gameRegistry.FindIdByGenerationLayer("surface"),
            .subsurfaceID   = gameRegistry.FindIdByGenerationLayer("subsurface"),
            .baseID         = gameRegistry.FindIdByGenerationLayer("base"),
            .waterID        = gameRegistry.FindIdByGenerationLayer("water")
        };
        ...
        auto structuralRules = gameRegistry.GetDecorationRules();

        for (int cx = 0; cx < m_widthChunks; ++cx) {
            for (int cz = 0; cz < m_depthChunks; ++cz) {
                Systems::ProcessMultiFeatureColumn(
                    std::span<Chunk>{m_chunks.data(), m_chunks.size() },
                    m_widthChunks,
                    m_depthChunks,
                    cx,
                    cz,
